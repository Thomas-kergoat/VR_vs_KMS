            ...
            else
            {
                ...
                if (UserManager.UserMeInstance == null)
                {
                    if (UserDeviceManager.GetDeviceUsed() == UserDeviceType.HTC)
                    {
                        ...
                        PhotonNetwork.Instantiate("Prefabs/" + playerPrefabVR.name, new Vector3(83f, 35.5f, 5f), Quaternion.identity, 0);
                    ...
                    else if (UserDeviceManager.GetDeviceUsed() == UserDeviceType.PC)
                    {
                        Debug.LogFormat("We are Instantiating LocalPlayer from {0}", SceneManagerHelper.ActiveSceneName);
                        // we're in a room. spawn a character for the local player. it gets synced by using PhotonNetwork.Instantiate
                        PhotonNetwork.Instantiate("Prefabs/" + playerPrefabPC.name, new Vector3(83f, 35.5f, 5f), Quaternion.identity, 0);
                    }
